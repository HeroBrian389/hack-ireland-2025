...
def insert_healthy_data_points(creds, days_back=7):
    """Insert multiple healthy data points over the past few days"""
    current_time = time.time()
    day_in_seconds = 24 * 60 * 60
    
    # Simulate stable, healthy vital signs over the past week
    for i in range(days_back):
        time_point = current_time - (i * day_in_seconds)
        time_ns = int(time_point * 1e9)
        
        # Weight stable (healthy BMI)
        weight = 70.0 + (i * 0.1)  # Minor daily fluctuations
...
if __name__ == '__main__':
    ...
    # Insert healthy data over the past week
    print("\nInserting healthy data over the past week...")
    insert_healthy_data_points(credentials)
    
    print("\nCurrent health status (most recent readings):")
    print("Height: 1.75 m")
    print("Weight: 70.0 kg (BMI: 22.9 - Normal)")
